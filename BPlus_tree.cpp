...
vector<int> CBPlusTree::rangeQuery(double up, double low, MatrixXd* tsCount)
{
	vector<int> rangeResult;
	int endIndex, tempR, tempC, tempID;
	bool terminal = false;
	...
	if (up < low)
	{
		...
		while(!terminal)
		{
			if(fn == bn)
			{
				...
				{
					tempID = ((CLeafNode*)fn)->getData(frontIndex);
					tempR = tempID / 1000;
					tempC = tempID % 1000;
					(*tsCount)(tempR,tempC)++;
					rangeResult.push_back(tempID);
					frontIndex++;
				}
				terminal = true;
			}
