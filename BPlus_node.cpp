...
{ 
	...
	KeyType finalKey = getKeyValue(left); 
	if(finalKey <= key)
	{
		left++;
	}
...
{ 
    CInternalNode* newNode = new CInternalNode();//right node after split  
    newNode->setKeyNum(MINNUM_KEY); 
    int i; 
    for (i=0; i<MINNUM_KEY; ++i)// copy key value 
    { 
        newNode->setKeyValue(i, m_KeyValues[i+MINNUM_CHILD]); 
    } 
    for (i=0; i<MINNUM_CHILD; ++i) // copy child's index 
    { 
        newNode->setChild(i, m_Childs[i+MINNUM_CHILD]); 
    } 
    setKeyNum(MINNUM_KEY);  //update the number of keys in left tree 
...
{
    int i; 
    for (i=getKeyNum(); i>keyIndex; --i)//let all childIndex of the parent node's key velue and the child tree's pointer backtowds 1 
    {    
...
{ 
    switch(d) 
    { 
    case LEFT:  // borrow from left node  
        { 
        ...
        break; 
    case RIGHT:  // borrow from right node 
        { 
...
{ 
    ...
    int i; 
    for (i=0; i<MINNUM_LEAF+1; ++i)// copy key value 
    { 
        newNode->setKeyValue(i, m_KeyValues[i+MINNUM_LEAF]); 
    } 
    for (i=0; i<MINNUM_LEAF+1; ++i)// copy data value 
    { 
